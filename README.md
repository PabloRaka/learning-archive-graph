...
> [!NOTE]
> Replace `/path/to/learning-archive-graph` with the absolute path to your cloned repository, and ensure you point `command` to the python executable within your backend virtual environment (e.g. `backend/venv/Scripts/python.exe` on Windows).

```json
{
  "mcpServers": {
    "learning-archive-graph": {
      "command": "/path/to/learning-archive-graph/backend/venv/bin/python",
      "args": [
...
```bash
openclaw mcp add learning-archive-graph --command "/path/to/learning-archive-graph/backend/venv/bin/python" --arg "/path/to/learning-archive-graph/mcp_server.py"
```
...
- **Type**: `command`
- **Command**: `/path/to/learning-archive-graph/backend/venv/bin/python /path/to/learning-archive-graph/mcp_server.py`
